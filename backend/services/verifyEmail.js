...
export const sendVerificationEmail = async (
  ...
  try {
    ...
    if (!user) {
      throw new Error(
        "Verification period expired for this email, please sign up again."
      );
    }
    ...
    if (
      !isRetry &&
      api_key === process.env.SEND_API &&
      process.env.SEND_API_BACKUP
    ) {
      console.log("Trying backup email service...");
      return await sendVerificationEmail(
        userEmail,
        process.env.EMAIL_USER_BACKUP,
        process.env.SEND_API_BACKUP,
        true
      );
    }
...
export const verifyCode = async (code, userEmail) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Error cleaning up Redis keys after verification:",
        redisError.message
      );
    }
